function [R] = getxlname()
    ...
    'name','getxlname',...
    'numbertitle','off',...
    'Color',bgcolor, 'name','Create or Update Excel Database');clf;drawnow;axis off;
    ht = text (-.1,.9, '\color{red}*\color{black}Enter Name of New Excel file: ',...
    ...
    'string','Enter',...
    'callback',{@pb_call});
    ht1 = text (-.1,.5, '\color{red}*\color{black}Over-Write Existing Excel file: ',...
    'units', 'normalized', 'FontSize',15);
    ...
    function [] = pb_call(varargin)
        ...
        if exist(get(S.ed,'string'),'file')==2  
            printf("***Failed %s already exists!***\n",get(S.ed,'string'));
            set(S.ed,'String','.xlsx');
        elseif length(get(S.ed,'string')) <= 5 || startsWith(get(S.ed,'string'),'.')
            printf("***Failed Invalid file name***\n",get(S.ed,'string'));
            set(S.ed,'String','.xlsx');
        else
            R = get(S.ed,'string');
