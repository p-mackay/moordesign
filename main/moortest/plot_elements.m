...
% command = 0 to plot to screen only
...
% get some idea of the scale/height of this mooring
for i=nh:-1:1,
   if H(4,i)~=1,
...
%      12345678911234567892123456789  (29 char long)
line0='0                            ';
%
for el=1:nh,
   line=line0;
   ...
   id=0; % very strange (?) way I find what each element is to plot it 
   [ne,le]=size(acrels);
   for ie=1:ne,
      if strcmp(line(1:16),acrels(ie,1:16)), % check acoustic releases
         id=ie; % this element identified as an acoustic release
      end
   end
   if id==0,
      [ne,le]=size(anchors);
      for ie=1:ne,
         if strcmp(line(1:16),anchors(ie,1:16)), % check anchors
            id=ie; % this element identified as an anchor
         end
      end
   ...
   if id==0,
      ...
      for ie=1:ne,
         if strcmp(line(1:16),chains(ie,1:16)), % check chains/shackles
            id=ie; % this element identified as a segment of chain 
         end
      end
   ...
   if id==0,
      ...
      for ie=1:ne,
         if strcmp(line(1:16),cms(ie,1:16)), % check current meters
            id=ie; % this element identified as a current meter
         end
      end
   ...
   if id==0,
      ...
      for ie=1:ne,
         if strcmp(line(1:16),floats(ie,1:16)), % check floatation
            id=ie; % this element identified as floatation
         end
      end
   ...
   if id==0,
      ...
      for ie=1:ne,
         if strcmp(line(1:16),miscs(ie,1:16)), % check miscs
            id=ie; % this element identified as a misc component
         end
      end
   ...
   if id==0,
      ...
      for ie=1:ne,
         if strcmp(line(1:16),wires(ie,1:16)),  % check wires/ropes
            id=ie; % this element identified as asegment of wire/rope
         end
      end
      if id~=0, % then this is a wire segment
          pltwire; 
          id=0; 
      end 
