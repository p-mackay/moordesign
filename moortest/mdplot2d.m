...
if (~isempty(H) & ~isempty(B) & ~isempty(Cd))|(~isempty(Ht) & ~isempty(Bt) & ~isempty(Cdt)),
            ...
            if nu>1, % then ask for time
                if its==1 & command~=21,
                    if isempty(h_edit_timemd),
                        ...
                        'Callback','mdplot2d(20)','FontSize',fs,...
                        'String',num2str(tmin),...
                        'Units','Normalized',...
                        'Position',[.6 .35 .35 .25]);
 ...
 if isempty(Ht),
     ...
     'Name','The Mooring',...
     'tag','mdplot2d');
     plot(X,Y,Z,'b');hold on;
     li=length(iobj);
     plot(X(iobj(2:li-1)),Y(iobj(2:li-1)),Z(iobj(2:li-1)),'or',...
     'Markersize',5,'Clipping','off');
     plot(X(iobj(1)),Y(iobj(1)),Z(iobj(1)),'or',...
     'Markersize',10,'MarkerFaceColor','r','Clipping','off');
     plot(X(iobj(li)),Y(iobj(li)),Z(iobj(li)),'^b',...
     'Markersize',8,'MarkerFaceColor','b','Clipping','off');
     if ~isempty(BCO), % then there are clamp-on devices
         plot(Xfco,Yfco,Zfco,'om',...
         'Markersize',5,'Clipping','off');
    end
            ...
            if min(ym)>-10, ym(1)=-10; end
                if max(ym)< 10, ym(2)=10; end
                    ...
                    if abs(max(Z)-max(z)) < 50, % then plot the ocean surface/waves
                        ...
                        box off;
                        plot(max(xm)*ones(size(swy)),swy,swzy,'b','Clipping','off');
                        plot(min(xm)*ones(size(swy)),swy,swzy,'b','Clipping','off');
                        plot(swx,max(ym)*ones(size(swx)),swzx,'b','Clipping','off');
                        plot(swx,min(ym)*ones(size(swx)),swzx,'b','Clipping','off');
...
elseif command == 4,
   ...
   vp=vp(:);
   if ~isempty(up),
       hup=plot(up,ymax1*ones(size(up)),z,'g');
       set(hup,'Color',[0 0.5 0]);
   end
   if ~isempty(vp),
       hvp=plot(xmax1*ones(size(vp)),vp,z,'m');
   end
   axis equal
   zlim=get(gca,'ZLim');
   zmax1=max(abs(zlim));
   ...
   if abs(max(Z)-max(z)) < 50, 
       ...
       swzy=(zm+0.75)-sqrt(1+sin(swy));
       box off;
       plot(max(xlim)*ones(size(swy)),swy,swzy,'b','Clipping','off');
       plot(min(xlim)*ones(size(swy)),swy,swzy,'b','Clipping','off');
